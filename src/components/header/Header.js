...
class Header extends PureComponent {
  ...
  addToCart = (product) => {
    const alreadyInCart = this.state.cartItems.some(
  ...
  increment = (items) => {
    this.setState((prevState) => ({
      cartItems: prevState.cartItems.map((item) => {
        return item.attributes === items.attributes
    ...
    if (count < 2) {
      this.setState((prevState) => ({
        cartItems: prevState.cartItems.filter(
          (item) => item.attributes !== items.attributes
        ),
      }))
      localStorage.setItem('cartItems', JSON.stringify(this.state.cartItems))
    } else {
      ...
      >
          ...
          <div className='nav-right'>
            <div className='currency-div'>
              <div onClick={changeCurrencyOverlay}>
                ...
                <img
                  ...
                  src={this.state.currencyOverlay ? downIcon : upIcon}
                  alt='currency icon'
                />
              </div>
            ...
            >
              {this.state.cartItems.length > 0 && (
                <div className='icon-quantity'>{this.state.quantity}</div>
              )}
              <img
                src={cart}
                alt='cart'
        ...
        {this.state.cartOverlay ? (
          <CartOverlay
            ...
            onMouseOver={this.showCartOverlay}
            quantity={this.state.quantity}
            onClickCartOverlay={onClickCartOverlay}
            closeCurrencyOverlay={closeCurrencyOverlay}
          />
        ) : (
          ''
        )}
        {this.state.itemInCart ? (
          <p className='itemInCartParagraph'>Item already in cart</p>
        ) : (
          this.state.itemAddedToCart && (
            <p className='itemAddedToCart'>Item Added to the cart </p>
          )
        )}
        <Routes>
          <Route
